...
struct limits<int32_t> {
  ...
  // N_min    = -2^29 + 36524       = -536,834,388,
  // Date     = 13 July      -1,469,803.
  static int64_t constexpr N_min    = -536834388;

  // N_max    = 2^29 - 1            =  536,870,911,
  // Date     = 18 October    1,469,902.
...
to_date(T N) noexcept {
  ...
  // quotient(N_1, 146097), which performs N_1 / 146097, yielding a correct
  // and smaller value than N_1, and of remainder(N_1, 146097), which simply
  // calculates N_1 % 146097, yielding a correct result in [0, 146097[.
  ...
  // shows that the results are correct for N >= -2^(S - 3) + 36524.
...
to_rata_die_opt(T Y_G, uint32_t M_G, uint32_t D_G) noexcept {
  ...
  // Overflow analysis is the same as to_rata_die().
